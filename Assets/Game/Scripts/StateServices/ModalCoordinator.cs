...
namespace Game.Scripts.StateServices
{
    ...
    {
        private readonly struct PendingModalRequest
        {
            public readonly ModalType Type;
            public readonly ModalPriority Priority;
            public readonly Action ShowAction;

            public PendingModalRequest(ModalType type, ModalPriority priority, Action showAction)
            {
                Type = type;
                Priority = priority;
        ...
        public ModalShowResult RequestShow(ModalType type, ModalPriority priority, Action showAction)
        {
            ...
            if (_queue.Any(request => request.Type == type))
                return ModalShowResult.AlreadyPending;

            if (_currentModal != ModalType.None)
            {
                _queue.Add(new PendingModalRequest(type, priority, showAction));
        ...
        private void ProcessQueue()
        {
            ...
            PendingModalRequest next = _queue[bestIndex];
